...
Uncented_all <- read_excel(here::here("hw05/Unscented_all.xlsx"))
View(Scented_all)
```
...
Scented_all %>% 
  ...
  group_by(Date) %>% 
    mutate(RatingMean = mean(Rating)) %>% 
   ggplot() +
  aes(as.Date(x = Date), y = RatingMean) +
  geom_point(alpha = .3, shape = 1, color = "purple") +
